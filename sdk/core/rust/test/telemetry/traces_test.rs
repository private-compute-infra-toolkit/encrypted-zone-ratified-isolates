...
use rust_core::telemetry::traces::setup_traces;
...
async fn test_trace_export() {
    ...
    // 2. Setup Telemetry
    // Note: setup_traces sets the global tracing subscriber.
    ...
    let provider =
        setup_traces("traces_test", &endpoint, 1.0).await.expect("Failed to setup telemetry");
